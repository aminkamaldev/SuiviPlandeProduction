...
currentAccount: any;
    ...
    loadAll() {
        this.jobExecutionService.query({
            page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/job-execution'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId (index: number, item: JobExecution) {
        return item.id;
    }
    ...
    registerChangeInJobExecutions() {
        this.eventSubscriber = this.eventManager.subscribe('jobExecutionListModification', (response) => this.loadAll());
    }
